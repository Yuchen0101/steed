  ...
  return (
      <>
        <View style={{flexDirection:"row", justifyContent:"space-between"}}>
          {monthScores.map((item, idx)=><ScoreCard key={idx} title={item.title} context={item.context} />)}
        </View>
        <Text style={{marginTop:30, marginBottom:10, fontStyle:"italic"}}>All Time</Text>
        <View style={{flexDirection:"row", justifyContent:"space-between"}}>
          {monthScores.map((item, idx)=><ScoreCard key={idx} title={item.title} context={item.context} />)}
        </View>
      </>
